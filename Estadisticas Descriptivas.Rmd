...
```{r grafico1_peso_pie, echo=FALSE, fig.width=10, fig.height=6}
# GRÁFICO 1: DISTRIBUCIÓN DEL PESO DEL RECIÉN NACIDO (PIE CHART)
tabla_peso <- Base_datos %>%
  count(Peso_delicado) %>%
  mutate(
    Grupo_Peso = ifelse(Peso_delicado == "Si", "Delicado", "Moderado"),
    Porcentaje = round(100 * n / sum(n), 1)
  )
...
plot_ly(
  ...
  plotly::layout(
    title = list(
      text = "<b>Gráfico 1. Distribución del Peso del Recién Nacido</b><br><sub>Bogotá - Segundo Semestre 2024</sub>",
      x = 0.5,
      font = list(size = 16)
    ),
    legend = list(
...
```{r grafico2_tiempo_gestacion, echo=FALSE, fig.width=10, fig.height=6}
# GRÁFICO 2: DISTRIBUCIÓN DEL TIEMPO DE GESTACIÓN (BAR CHART)
tabla_tg <- Base_datos %>%
  count(Tiempo_gestación) %>%
...
plot_ly(
  ...
  plotly::layout(
    title = list(
      text = "<b>Gráfico 2. Distribución del Tiempo de Gestación</b><br><sub>Bogotá - Segundo Semestre 2024</sub>",
      x = 0.5,
      font = list(size = 16)
    ),
    xaxis = list(
...
```{r grafico4_edad_madre, echo=FALSE, fig.width=10, fig.height=6}
# GRÁFICO 4: DISTRIBUCIÓN POR EDAD DE LA MADRE (BAR CHART)
tabla_edad <- Base_datos %>%
  count(Edad_madre) %>%
  mutate(Porcentaje = round(100 * n / sum(n), 1)) %>%
...
plot_ly(
  tabla_edad,
  x = ~reorder(Edad_madre, Porcentaje),
  y = ~Porcentaje,
  type = 'bar',
  text = ~paste0(Porcentaje, "%"),
  textposition = 'outside',
  ...
  plotly::layout(
    title = list(
      text = "<b>Gráfico 4. Distribución por Grupos de Edad Materna</b><br><sub>Bogotá - Segundo Semestre 2024</sub>",
      x = 0.5,
      font = list(size = 16)
    ),
    xaxis = list(
...
```{r grafico5_tiempo_vs_peso, echo=FALSE, fig.width=10, fig.height=6}
# GRÁFICO 5: RELACIÓN TIEMPO DE GESTACIÓN VS PESO (BARRAS APILADAS)
...
plot_ly(
  ...
  plotly::layout(
    barmode = 'stack',
    title = list(
      text = "<b>Gráfico 5. Tiempo de Gestación según Grupo de Peso</b><br><sub>Bogotá - Segundo Semestre 2024</sub>",
      x = 0.5,
      font = list(size = 16)
    ),
    xaxis = list(
    ...
    legend = list(
      title = list(text = "<b>Grupo de Peso</b>"),
      orientation = "v",
      x = 1.02,
      y = 1
    ),
...
```{r grafico6_resumen_donut, echo=FALSE, fig.width=10, fig.height=6}
# GRÁFICO 6: RESUMEN GENERAL (DONUT CHART)
tabla_resumen_graf <- Base_datos %>%
  count(Peso_delicado) %>%
  mutate(
    Categoria = ifelse(Peso_delicado == "Si", "Peso Delicado", "Peso Moderado"),
    Porcentaje = round(100 * n / sum(n), 1)
...
plot_ly(
  ...
  plotly::layout(
    title = list(
      text = "<b>Gráfico 6. Resumen General de Categorías de Peso</b><br><sub>Bogotá - Segundo Semestre 2024</sub>",
      x = 0.5,
      font = list(size = 16)
    ),
    legend = list(
