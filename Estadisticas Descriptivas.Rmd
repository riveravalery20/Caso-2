...
# Función para crear tablas cruzadas consistentes
crear_tabla_cruzada <- function(data, variable, titulo) {
  data %>%
    count({{variable}}, Peso_delicado) %>%
    group_by({{variable}}) %>%
    mutate(Total_Grupo = sum(n)) %>%
    ungroup() %>%
    filter(Peso_delicado == "Si") %>%
    mutate(Porcentaje_Delicado = round(100 * n / Total_Grupo, 1)) %>%
    select({{variable}}, n, Porcentaje_Delicado) %>%
    rename("Frecuencia Delicado" = n, "% Delicado" = Porcentaje_Delicado)
}

# 5a: Tiempo de gestación vs Peso
tabla_cruzada_tiempo <- crear_tabla_cruzada(Base_datos, Tiempo_gestación, "TIEMPO DE GESTACIÓN")

kable(tabla_cruzada_tiempo, 
      align = c("l", "r", "r"),
      col.names = c("Tiempo Gestación", "Frecuencia", "% Delicado"),
      caption = "TABLA 5A. DISTRIBUCIÓN CRUZADA - TIEMPO DE GESTACIÓN") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"),
    full_width = FALSE,
    font_size = 11
...
tabla_cruzada_parto <- crear_tabla_cruzada(Base_datos, Tipo_parto, "TIPO DE PARTO")

kable(tabla_cruzada_parto, 
      align = c("l", "r", "r"),
      col.names = c("Tipo Parto", "Frecuencia", "% Delicado"),
      caption = "TABLA 5B. DISTRIBUCIÓN CRUZADA - TIPO DE PARTO") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"),
    full_width = FALSE,
    font_size = 11
...
tabla_cruzada_edad <- crear_tabla_cruzada(Base_datos, Edad_madre, "EDAD DE LA MADRE")

kable(tabla_cruzada_edad, 
      align = c("l", "r", "r"),
      col.names = c("Edad Madre", "Frecuencia", "% Delicado"),
      caption = "TABLA 5C. DISTRIBUCIÓN CRUZADA - EDAD DE LA MADRE") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"),
    full_width = FALSE,
    font_size = 11
